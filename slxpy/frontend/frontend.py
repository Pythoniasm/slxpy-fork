...
from slxpy.common.context import Context, FieldMode, Module, ModelClass, Type, Field, Method, TypeContainer, Enumerator
...
def adapt_metadata(workdir: Path):
    ...
    with (workdir / C.model_config_name).open("rb") as f: model_config = ModelConfig.load(f)
    with (workdir / C.env_config_name).open("rb") as f: env_config = EnvConfig.load(f)
    ...
    context = Context(
        sources=[
            ...
            os.path.relpath(x, modeldir).replace("\\","/") for x in (
