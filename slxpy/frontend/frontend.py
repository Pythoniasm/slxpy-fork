...
from slxpy.common.context import (
    Context,
    FieldMode,
    Module,
    ModelClass,
...
def adapt_metadata(workdir: Path):
    ...
    doc = (
        ...
        if model_config.info.description == InfoConfig.AUTO
        else model_config.info.description
    )
    author = (
        metadata["author"]
        if model_config.info.author == InfoConfig.AUTO
        else model_config.info.author
    )
    version = (
        metadata["version"]
        if model_config.info.version == InfoConfig.AUTO
        else model_config.info.version
    )
    license = (
        ""
        if model_config.info.license == InfoConfig.NO_LICENSE
        else model_config.info.license
    )
