...
class AssetInfo:
    name: str
    template: str
    condition: Callable[[Context], bool] = DEFAULT_CONDITION
    overwrite: bool = True
...
assets: List[AssetInfo] = [
    ...
    AssetInfo(
        name="test_extension.py", template="test_extension.py.jinja", overwrite=False
    ),
]
...
def render(workdir: Path, debug: bool = False):
    ...
    optional_headers = [
        asset.name for asset in active_assets if asset.name.endswith(".h")
    ]
    ...
    shutil.copytree(
        include_thirdparty / "nlohmann", include_dst / "nlohmann", dirs_exist_ok=True
    )
